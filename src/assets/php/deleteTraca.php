...
$con = new Connexion();
$con->createQuery('SET foreign_key_checks = 0');
//Process
...
foreach ($process['OPERATION_GROUP'] as $keyOperationGroup => $operationGroup) {
  ...
  if ($detailedOperationsList) {
    foreach ($detailedOperationsList as $keyDetailedOperation => $detailedOperation) {
      ...
      $sql = "DELETE FROM t_prod_suboperations WHERE ID_PROD_PROCESS = :idProdProcess AND ID_PROCESS_SUBOPE = :detailedOperationId ";
      $query = $con->createQuery($sql, ['idProdProcess' => $prodProcess['ID_PROD_PROCESS'], 'detailedOperationId' => $detailedOperation['ID_SUB_OPERATION']]);
      ...
      if ($steps) {
        foreach ($steps as $keyStep => $step) {
          ...
          $sql = "DELETE FROM t_prod_suboperation_steps WHERE ID_PROD_SUBOP = :idProdSubOperation AND ID_STEP = :idProcessStep";
          $query = $con->createQuery($sql, ['idProdSubOperation' => $prodSubOperation['ID_PROD_SUBOP'], 'idProcessStep' => $step['ID_STEP']]);
          ...
          if ($prodSubOperationStep) {
            ...
            $sql = "DELETE FROM t_prod_traca_user WHERE ID_PROD_STEP = :idProdStep";
          ...
          if ($tracasList) {
            ...
            foreach ($tracasList as $keyTraca => $tracas) {
              ...
              if ($tracas) {
                ...
                $sql = "DELETE FROM t_prod_traca WHERE ID_TRACA = :idTraca AND ID_PROD_STEP = :idProdStep";
                $query = $con->createQuery($sql, ['idTraca' => $tracas['ID_TRACA'], 'idProdStep'=> $prodSubOperationStep['ID_PROD_STEP']]);
                ...
                foreach ($traca as $keyTracaDet => $tracaDeta) {
                  ...
                  $sql = "DELETE FROM  $prodTracaTypeTable WHERE ID_PROD_TRACA = :idProdTraca AND $idTracaParam = :idTracaControle";
                  $query = $con->createQuery($sql, ['idProdTraca' => $prodTraca['ID_PROD_TRACA'], 'idTracaControle' => $tracaDeta[$idTracaParam]]);
...
// }
$con->createQuery('SET foreign_key_checks = 1');
